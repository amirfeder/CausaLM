...
You can then train on the pregenerated data using [`finetune_on_pregenerated.py`](IMA/finetune_on_pregenerated.py), and pointing it to the folder created by [`pregenerate_training_data.py`](IMA/pregenerate_training_data.py). Note that you should use the same `bert_model` and case options for both! Also note that `max_seq_len` does not need to be specified for the [`finetune_on_pregenerated.py`](IMA/finetune_on_pregenerated.py) script, as it is inferred from the training examples.
