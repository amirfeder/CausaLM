from os import getenv
from utils import count_num_cpu_gpu

HOME_DIR = getenv('HOME', "/home/{}".format(getenv('USER')))

CAUSALM_DIR = "" # This must be set to the path where the CausaLM project and datasets reside

DATA_DIR = f"{CAUSALM_DIR}/Data"
EXPERIMENTS_DIR = f"{CAUSALM_DIR}/Experiments"
SENTIMENT_DATA_DIR = f"{DATA_DIR}/Sentiment"
SENTIMENT_EXPERIMENTS_DIR = f"{EXPERIMENTS_DIR}/Sentiment"
SENTIMENT_RAW_DATA_DIR = f"{SENTIMENT_DATA_DIR}/Raw"
SENTIMENT_DOMAINS = ("movies", "books", "electronics", "kitchen", "dvd")
ALL_SENTIMENT_DOMAINS = ("movies", "books", "electronics", "kitchen", "dvd", "unified")
SENTIMENT_MODES = ["IMA", "MLM", "OOB"]
BERT_PRETRAINED_MODEL = 'bert-base-cased'
MAX_SENTIMENT_SEQ_LENGTH = 384
MAX_POMS_SEQ_LENGTH = 32
RANDOM_SEED = 212
NUM_CPU = count_num_cpu_gpu()[0]
NUM_GPU = 1
SENTIMENT_IMA_DATA_DIR = f"{DATA_DIR}/Sentiment/IMA"
SENTIMENT_IMA_PRETRAIN_DATA_DIR = f"{SENTIMENT_IMA_DATA_DIR}/Pretrain"
SENTIMENT_MLM_DATA_DIR = f"{DATA_DIR}/Sentiment/MLM"
SENTIMENT_MLM_PRETRAIN_DATA_DIR = f"{SENTIMENT_MLM_DATA_DIR}/Pretrain"
POMS_DATA_DIR = f"{DATA_DIR}/POMS"
POMS_RAW_DATA_DIR = f"{POMS_DATA_DIR}/Equity-Evaluation-Corpus"
POMS_GENDER_DATA_DIR = f"{POMS_DATA_DIR}/Gender"
POMS_GENDER_DATASETS_DIR = f"{POMS_GENDER_DATA_DIR}/Datasets"
POMS_GENDER_PRETRAIN_DATA_DIR = f"{POMS_GENDER_DATA_DIR}/Pretrain"
POMS_RACE_DATA_DIR = f"{POMS_DATA_DIR}/Race"
POMS_RACE_DATASETS_DIR = f"{POMS_RACE_DATA_DIR}/Datasets"
POMS_RACE_PRETRAIN_DATA_DIR = f"{POMS_RACE_DATA_DIR}/Pretrain"
POMS_MLM_DATA_DIR = f"{POMS_DATA_DIR}/MLM"
POMS_EXPERIMENTS_DIR = f"{EXPERIMENTS_DIR}/POMS"
SENTIMENT_TOPICS_DATA_DIR = f"{SENTIMENT_DATA_DIR}/Topics"
SENTIMENT_TOPICS_DATASETS_DIR = f"{SENTIMENT_RAW_DATA_DIR}/unified"
SENTIMENT_TOPICS_PRETRAIN_DIR = f"{SENTIMENT_TOPICS_DATA_DIR}/Pretrain"
SENTIMENT_TOPICS_PRETRAIN_DATA_DIR = f"{SENTIMENT_TOPICS_PRETRAIN_DIR}/data"
SENTIMENT_TOPICS_PRETRAIN_MLM_DIR = f"{SENTIMENT_TOPICS_PRETRAIN_DIR}/MLM"
SENTIMENT_TOPICS_PRETRAIN_ITX_DIR = f"{SENTIMENT_TOPICS_PRETRAIN_DIR}/ITX"
SENTIMENT_TOPICS_DOMAIN_TREAT_CONTROL_MAP_FILE = f"{CAUSALM_DIR}/Sentiment_Topics/domain_treat_control_topics.json"
