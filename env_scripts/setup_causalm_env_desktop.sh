...
conda install -y tensorflow keras cudnn
conda deactivate
echo "Conda AllenNLP Env Setup"
conda create --name causalm_allennlp
conda activate causalm_allennlp
conda install -y pip tensorflow cudnn mkl pandas tabulate tqdm cython seaborn beautifulsoup4 scikit-learn numpy scipy nltk lxml matplotlib
...
conda deactivate
# echo "Clone MOOD git repository"
# mkdir -p ~/dev
# cd ~/dev/
# git clone https://github.com/nadavo/Mood.git
echo "export CAUASLM_REPO=\$HOME/dev/CausaLM/" >> ~/.bash_profile
...
# echo "alias 'mood_env'='source ~/env/mood/bin/activate && export PYTHONPATH=\$MOOD_REPO:\$PYTHONPATH && cd \$MOOD_REPO'" >> ~/.bash_profile
